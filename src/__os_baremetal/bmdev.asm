...
b_print_string_with_color	equ 0x00000000001001C8	; Displays text in colour. IN: RSI = message location (zero-terminated string), CL = colour
b_print_char_with_color		equ 0x00000000001001D0	; Displays a char in colour. IN: AL = char to display, CL = colour
b_ethernet_tx			equ 0x00000000001001D8	; Transmit a packet via Ethernet. IN: RSI = Memory location where data is stored, RDI = Pointer to 48 bit destination address, BX = Type of packet (If set to 0 then the EtherType will be set to the length of data), CX = Length of data
b_ethernet_rx			equ 0x00000000001001E0	; Polls the Ethernet card for received data. IN: RDI = Memory location where packet will be stored. OUT: RCX = Length of packet
b_mem_allocate			equ 0x00000000001001E8	; Allocates the requested number of 2 MiB pages. IN: RCX = Number of pages to allocate. OUT: RAX = Starting address, RCX = Number of pages allocated (Set to the value asked for or 0 on failure)
b_mem_release			equ 0x00000000001001F0	; Frees the requested number of 2 MiB pages. IN: RAX = Starting address, RCX = Number of pages to free. OUT: RCX = Number of pages freed
b_mem_get_free			equ 0x00000000001001F8	; Returns the number of 2 MiB pages that are available. OUT: RCX = Number of free 2 MiB pages
b_smp_numcores			equ 0x0000000000100200	; Returns the number of cores in this computer. OUT: RAX = number of cores in this computer
